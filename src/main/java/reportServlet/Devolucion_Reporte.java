...
/**
 * Servlet implementation class Devolucion_Reporte
 */
@SuppressWarnings("deprecation")
@WebServlet("/dashboard/inventario/ReporteDevoluciones")
public class Devolucion_Reporte extends HttpServlet {
				...
				if (!val.verificarsesion(request.getSession().getAttribute("id").toString(), usuario,
					...
					try {
					    ...
					    if(id != null && !id.trim().equals("")) {
						   	query = "SELECT I.UsuarioExterno AS UsuarioExt, (U.Nombres || ' ' || U.Apellidos) AS UsuarioInt, U.Id, I.FechaReporte FROM Inventarios AS I "
						   	...
						   	if(usuariosbd.size() > 0) {
								...
								if(accesos.get(0).equals("0") && !usuariosbd.get(2).equals(request.getSession().getAttribute("id"))) {
									try
								    {
								    	JasperReport reporte = (JasperReport) JRLoader.loadObjectFromFile(getServletContext().getRealPath("resources/reportes/Vacio.jasper"));
								    	JasperPrint jasperPrint = JasperFillManager.fillReport(reporte, null);
								    ...
								    try
								    {
										...
										if (FHoy.equals(usuariosbd.get(3))) {
											JasperReport reporte = (JasperReport) JRLoader.loadObjectFromFile(getServletContext().getRealPath("resources/reportes/Devolucion_sFecha.jasper"));
											...
											JasperReport reporte = (JasperReport) JRLoader.loadObjectFromFile(getServletContext().getRealPath("resources/reportes/Devolucion.jasper"));
									    	JasperPrint jasperPrint = JasperFillManager.fillReport(reporte, parametros, val.Conexion());
									    	JRExporter exporter = new JRPdfExporter();
									    	exporter.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
									    	exporter.setParameter(JRExporterParameter.OUTPUT_STREAM, out);
