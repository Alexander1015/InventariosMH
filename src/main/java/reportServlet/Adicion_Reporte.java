...
/**
 * Servlet implementation class Adicion_Reporte
 */
@SuppressWarnings("deprecation")
@WebServlet("/dashboard/inventario/ReporteAdiciones")
public class Adicion_Reporte extends HttpServlet {
				...
				if (!val.verificarsesion(request.getSession().getAttribute("id").toString(), usuario,
					...
					try {
					   ...
					   if(id != null && !id.trim().equals("")) {
						   	query = "SELECT I.Autorizacion, (U.Nombres || ' ' || U.Apellidos) AS UsuarioInt, I.Caso, U.Id, I.FechaReporte, I.Referencia FROM Inventarios AS I "
						   	...
						   	if(usuariosbd.size() > 0) {
								...
								if(accesos.get(0).equals("0") && !usuariosbd.get(3).equals(request.getSession().getAttribute("id"))) {
									try
								    {
								   	...
								   	if(usuariosbd.get(0) != null && !usuariosbd.get(0).trim().equals("")) parametros.put("Usuario", "I.Autorizacion");
									...
									if(usuariosbd.get(5) != null && !usuariosbd.get(5).trim().equals("")) parametros.put("Referencia", "I.Referencia");
								    ...
								    try
								    {
										...
										if (FHoy.equals(usuariosbd.get(4))) {
											JasperReport reporte = (JasperReport) JRLoader.loadObjectFromFile(getServletContext().getRealPath("resources/reportes/Adicion_sFecha.jasper"));
											...
											JasperReport reporte = (JasperReport) JRLoader.loadObjectFromFile(getServletContext().getRealPath("resources/reportes/Adicion.jasper"));
									    	JasperPrint jasperPrint = JasperFillManager.fillReport(reporte, parametros, val.Conexion());
									    	JRExporter exporter = new JRPdfExporter();
									    	exporter.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
									    	exporter.setParameter(JRExporterParameter.OUTPUT_STREAM, out);
