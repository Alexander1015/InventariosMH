...
/**
 * Servlet implementation class Ingreso_Reporte
 */
@SuppressWarnings("deprecation")
@WebServlet("/dashboard/inventario/ReporteIngresos")
public class Ingreso_Reporte extends HttpServlet {
				...
				if (!val.verificarsesion(request.getSession().getAttribute("id").toString(), usuario,
					...
					try {
					   ...
					   if(id != null && !id.trim().equals("")) {
						   	query = "SELECT I.Autorizacion AS Autorizacion, (U.Nombres || ' ' || U.Apellidos) AS UsuarioInt, I.Caso, U.Id, I.FechaReporte, I.Referencia FROM Inventarios AS I "
								...
								if(accesos.get(0).equals("0") && !usuariosbd.get(3).equals(request.getSession().getAttribute("id"))) {
								    ...
								    try
								    {
										...
										if (FHoy.equals(usuariosbd.get(4))) {
											JasperReport reporte = (JasperReport) JRLoader.loadObjectFromFile(getServletContext().getRealPath("resources/reportes/Ingreso_sFecha.jasper"));
											...
											JasperReport reporte = (JasperReport) JRLoader.loadObjectFromFile(getServletContext().getRealPath("resources/reportes/Ingreso.jasper"));
									    	JasperPrint jasperPrint = JasperFillManager.fillReport(reporte, parametros, val.Conexion());
									    	JRExporter exporter = new JRPdfExporter();
									    	exporter.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
									    	exporter.setParameter(JRExporterParameter.OUTPUT_STREAM, out);
