...
/**
 * Servlet implementation class Adiciones_Productos
 */
@WebServlet("/dashboard/inventario/adicion")
public class Adiciones_Productos extends HttpServlet {
						...
						if(accesos.get(0).equals("0")) {
							...
							if (producto != null && !producto.trim().equals("")) {
								query = "SELECT I.Id FROM Acciones AS A INNER JOIN Inventarios AS I ON I.Id = A.IdInventario WHERE I.IdUsuario = '"
										...
										+ "' AND I.Accion = '2' AND I.Estado = '0' ORDER BY Id DESC LIMIT 1";
								...
								if (inventario != null && !inventario.get(0).trim().equals("")) {
									...
									query = "UPDATE Inventarios SET Fecha = '" + Fecha + "' WHERE Id = '"
											+ inventario.get(0) + "'";
									val.executeQuery(query);
									request.getSession().setAttribute("succAddP", "Proceso realizado correctamente.");
									response.sendRedirect("./add.jsp");
								} else {
									request.getSession().setAttribute("mensaddinventario",
											"El accesorio/dispositivo a quitar no es el indicado.");
									response.sendRedirect("./add.jsp");
								}
							} else {
								request.getSession().setAttribute("mensaddinventario", "El accesorio/dispositivo a quitar no es el indicado.");
								response.sendRedirect("./add.jsp");
							}
						}
						...
						if(accesos.get(0).equals("0")) {
							...
							if (productos != null) {
								...
								query = "SELECT Id FROM Inventarios WHERE IdUsuario = '"
										...
										+ "' AND Accion = '2' AND Estado = '0' ORDER BY Id DESC LIMIT 1";
								...
								if (existId == 0) {
									query = "INSERT INTO Inventarios (IdUsuario, Accion, Estado, Fecha) VALUES ('"
											+ request.getSession().getAttribute("id") + "', '2', '0', '" + Fecha + "')";
									val.executeQuery(query);
								...
								if(error == 0) request.getSession().setAttribute("succAddP", "Proceso realizado correctamente.");
								else request.getSession().setAttribute("succAddP", "Es posible que solo algunos procesos se hayan realizado correctamente.");
								response.sendRedirect("./add.jsp");
							} else {
