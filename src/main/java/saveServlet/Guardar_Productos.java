						...
						if(accesos.get(0).equals("0")) {
							...
							if(auto == "1" || (autoriza != null && !autoriza.trim().equals(""))) {
								...
								if(auto == "1" || (cantautor > 1 && cantautor <= 100 && !val.existNumeros(autoriza))) {
									...
									request.getSession().setAttribute("mensinsmodinventario", "No ha ingresado el que autorizó correctamente.");
									response.sendRedirect("./insertar");
								}
							}
							else {
								request.getSession().setAttribute("mensinsmodinventario", "No ha ingresado el que autorizó este movimiento.");
								response.sendRedirect("./insertar");
							}
						}
