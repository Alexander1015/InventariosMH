...
<html lang="es">
    <head>
         <% out.print(layout.header("Historial", 2)); %>
    </head>
    <body>
       <%
    		String iduser = (String)session.getAttribute("id");
			out.print(layout.navbar("Historial", iduser, (String)session.getAttribute("token"), 2));
			...
			if(session.getAttribute("tablahisto") != null) {
				datos = (ArrayList<ArrayList<String>>) session.getAttribute("tablahisto");
				session.setAttribute("tablahisto", null);
			}
			String search = "";
			if(session.getAttribute("sinputhisto") != null && !session.getAttribute("sinputhisto").equals("")) {
				search = (String) session.getAttribute("sinputhisto");
			}
			String saccesorio = "";
			if(session.getAttribute("sinputhistocce") != null && !session.getAttribute("sinputhistocce").equals("")) {
				saccesorio = (String) session.getAttribute("sinputhistocce");
			}
			String FAntes = "", FDespues = "";
			if(session.getAttribute("sinputhistontes") != null && !session.getAttribute("sinputhistontes").equals("")) {
				FAntes = (String) session.getAttribute("sinputhistontes");
			}
			else FAntes = "";
			if(session.getAttribute("sinputhistodespues") != null && !session.getAttribute("sinputhistodespues").equals("")) {
				FDespues = (String) session.getAttribute("sinputhistodespues");
			}
			String accion = "";
			if(session.getAttribute("sinputhistoccion") != null && !session.getAttribute("sinputhistoccion").equals("")) {
				accion = (String) session.getAttribute("sinputhistoccion");
			}
			Date dNow = new Date();
			SimpleDateFormat Fsdf = new SimpleDateFormat ("YYYY-MM-dd");
			String max = Fsdf.format(dNow);
			query = "SELECT Nombres, Apellidos FROM Usuarios WHERE Id = '" + (String)session.getAttribute("id") + "'";
        ...
        <div id="panPrincipal" class="col col-md-12">
            <div class="row">
                <div class="col col-sm-12">
                	...
                	%>
                    <p class="encabezadodash">MI HISTORIAL DE PROCESOS
                    	<span class="subencab"> / <% out.print((nombre + " " + apellido).toUpperCase()); %></span>
                    </p>
                    <div class="grp-buscar col col-12">
                        <form class="col col-12 col-sm-12 col-md-11 col-lg-10" action="historial" method="POST">
	                        <div class="row grp-buscar">
	                           			...
	                           			<tbody>
	                              			<tr class="col col-12">
	                              				...
	                              				<td class="col col-9 col-sm-9">
                           							<input autocomplete="off" type="date" class="form-control dtfecha" id="dtInicio" name="dtInicio" value="<%= FAntes %>" min="2020-01-01" max="<%= max %>" onkeydown="return false">
	                              				</td>
	                              			</tr>
	                              			<tr class="col col-12">
	                              				<td class="col col-3 col-sm-3"><p>Hasta:</p></td>
	                              				<td class="col col-9 col-sm-9">
	                              					<input autocomplete="off" type="date" class="form-control dtfecha" id="dtFin" name="dtFin" value="<%= FDespues %>" min="2020-01-01" max="<%= max %>" onkeydown="return false">
	                                           </td>
	                              			</tr>
	                              		</tbody>
                       				</table>
                        		...
                        		<div class="row col col-12 col-md-2 divcentrado">
                        			<table class="col col-12">
	                           			<tbody>
	                              			...
	                              			<tr class="col col-12">
	                              				<td class="col">
		                              				<a href="../borrar?view=Historial" class="btn btn-ingresar areporte" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Borrar los datos ingresados.">
                           	...
                           	<%
	    	                  	//Errores de busqueda
	    						if(session.getAttribute("menshistorial") != null && !session.getAttribute("menshistorial").equals("")) {
	    							out.print("<div class='margsup'></div>");
	    							out.print(val.mostrarAlert(0, session.getAttribute("menshistorial") .toString()));
	    							session.setAttribute("menshistorial", null);
	    						}
                            %>
                            <div class="expli-just">
		                       	<p>
		                       		Realice una búsqueda de los procesos realizados en la sección del Inventario de Accesorios/Dispositivos (excluyendo el apartado del manejo de clasificaciones) realizados por el usuario actual, si no ha realizado una búsqueda (o deja en blanco todos los datos que se piden) se mostrarán los últimos 10 procesos en el sistema (no consultas).
		                       	</p>
	                       	</div>
	                     </form>
	                 </div>
                    <%
						if(session.getAttribute("existablahisto") != null)
						{
	                        ...
	                        <table id="registrosbit" data-page-length="10" class="table table-hover table-bordered table-sm align-middle">
								<thead class="tableheader">
	                                <tr>
	                                    ...
	                                    <th scope="col">ASIGNADO A
	                                    <th scope="col">UBICACIÓN REFERIDA</th>
								  			...
								  			if(datos != null) {
										  		...
										  		for (i = 0; i < elementos; i++) {
										  			if(!registro.equals(datos.get(i).get(0))) {
										  				out.print("<tr>");
										  				num++;
										  				if(!datos.get(i).get(1).equals("0")) {
										  					//Arreglando la fecha
													  		String fecha = datos.get(i).get(2);
												  			...
												  			out.print("<td><span class='oculto'>" + datos.get(i).get(2) + "</span>" + fecha + "</td>");
												  			//Arreglamos la hora
											  				String hora = datos.get(i).get(3);
												  			int canthora = Integer.parseInt(datos.get(i).get(3).charAt(0) + "" + datos.get(i).get(3).charAt(1));
												  			if(canthora > 12) {
												  			...
												  			out.print("<td><span class='oculto'>" + datos.get(i).get(3) + "</span>" + hora + "</td>");
											  			}
											  			else {
											  				out.print("<td>-</td>");
											  				out.print("<td>-</td>");
											  			}
										  				out.print("<td>" + ((datos.get(i).get(6) != null && !datos.get(i).get(6).trim().equals("")) && ((datos.get(i).get(8) != null && !datos.get(i).get(8).trim().equals(""))) ? "No. " + datos.get(i).get(6) + " / " + datos.get(i).get(8) : "-") + "</td>");
										  				if(!datos.get(i).get(1).equals("0") && !datos.get(i).get(1).equals("5")) {
											  				out.print("<td>" + ((datos.get(i).get(4) != null && !datos.get(i).get(4).trim().equals("")) ? datos.get(i).get(4) : nombre + " " + apellido) + "</td>");
										  				}
										  				else {
										  					out.print("<td>-</td>");
										  				}
										  				out.print("<td>" + ((datos.get(i).get(5) != null && !datos.get(i).get(5).trim().equals("")) ? datos.get(i).get(5) : "-") + "</td>");
										  				if(!datos.get(i).get(1).equals("0") && !datos.get(i).get(1).equals("5") && !datos.get(i).get(1).equals("4")) {
											  				out.print("<td>" + ((datos.get(i).get(7) != null && !datos.get(i).get(7).trim().equals("")) ? datos.get(i).get(7) : nombre + " " + apellido) + "</td>");
										  				}
										  				else {
										  					out.print("<td>-</td>");
										  				}
										  				out.print("<td>");
											  			if(datos.get(i).get(1).equals("0")) out.print("Consultas");
											  			else if(datos.get(i).get(1).equals("1")) out.print("Ingresos");
											  			else if(datos.get(i).get(1).equals("2")) out.print("Adiciones");
											  			else if(datos.get(i).get(1).equals("3")) out.print("Retiros");
											  			else if(datos.get(i).get(1).equals("4")) out.print("Devoluciones");
											  			else if(datos.get(i).get(1).equals("5")) out.print("Modificaciones");
											  			else out.print("N/A");
											  			out.print("</td>");
											  			out.print("<td>");
							  							...
							  							if(datos.get(i).get(1).equals("0") || datos.get(i).get(1).equals("5")) {
							  								...
							  								//Diferentes reportes
							  								if(datos.get(i).get(1).equals("1")) out.print("../inventario/ReporteIngresos?id=" + datos.get(i).get(0));
							  								else if(datos.get(i).get(1).equals("2")) out.print("../inventario/ReporteAdiciones?id=" + datos.get(i).get(0));
							  								else if(datos.get(i).get(1).equals("3")) out.print("../inventario/ReporteRetiros?id=" + datos.get(i).get(0));
							  								else if(datos.get(i).get(1).equals("4")) out.print("../inventario/ReporteDevoluciones?id=" + datos.get(i).get(0));
							  								else out.print("");
											  			...
											  			out.print("<h5 class=\"modal-title\" id=\"descript" + datos.get(i).get(0) + "Label\">Descripción del Registro ingresado" + ((datos.get(i).get(4) != null && !datos.get(i).get(4).trim().equals("")) ? (" para <b class='bbig'>" + datos.get(i).get(4) +  "</b>") : "") + "</h5>");
											  			...
											  			if(datos.get(i).get(1).equals("0")) {
											  			...
											  			else if(datos.get(i).get(1).equals("5")) {
												  			...
												  			if(datos.get(i).get(9) != null && !datos.get(i).get(9).trim().equals("")) {
												  				out.print("<b>Comentario añadido:</b> " + datos.get(i).get(9) + "<br><br>");
												  			}
												  			...
												  			for(int j = 0; j < cantidad; j++) {
										  						...
										  						out.print("</b></li>");
												  				out.print("<ul>");
												  				if(datos.get(i).get(1).equals("1")) out.print("<li>Cantidad ingresada: <b>" + datosProd.get(j).get(2) + "</b></li>");
												  				else if(datos.get(i).get(1).equals("2")) {
												  				...
												  				else if(datos.get(i).get(1).equals("3")) {
												  					out.print("<li>Cantidad anterior: <b>" + datosProd.get(j).get(2) + "</b></li>");
												  				...
												  				else if(datos.get(i).get(1).equals("4")) {
												  					out.print("<li>Cantidad anterior: <b>" + datosProd.get(j).get(2) + "</b></li>");
